...
### 2026-06-10：按 2026.6.5新版(6.10) Excel 橙黄色标注更新产业链服务链接

需求：以 `胶州市家居产业服务“一类事”线上框架 - 2026.6.5新版(6.10).xls` 为准，仅调整“板块二、产业链服务”中橙黄色标注的跳转链接，其他功能和数据保持不变。

处理：

- 已解析“板块二、产业链服务”中填充色 `FFCC00` 的链接型单元格，共 27 个。
- 已最小化更新 `backend/data/events.db` 中 `industry_chain` 对应 27 个 `cd_content.link_url`：
  - 上游 11 个；
  - 中游 11 个；
  - 下游 5 个。
- 仅更新 `link_url` 和 `updated_at`，不改节点标题、层级、正文、`link_label` 或其他模块数据。
- 修改前已备份数据库：`backend/data/events.db.bak-20260610-160339-before-20260610-orange-link-update`。

验证：

- Excel 橙黄色 27 个链接项与数据库复核：`mismatchCount=0`、`unresolvedCount=0`。
- 重启本地 3001 后端后，公开接口 `GET /api/cd/public/modules/industry_chain/tree` 已返回新链接。

### 2026-06-10：首页背景图、产业链链接校验和前台标题去序号

需求：将家居产业首页背景图替换为 `素材/微信图片_20260610100529_118_147.jpg`；产业链服务跳转链接以 `胶州市家居产业服务“一类事”线上框架 - 2026.6.6新版.xls` 中黄色标注内容为准；前台展示时去掉节点标题开头的数字序号（如 `1.1基础服务` 展示为 `基础服务`）。

处理：

- 已将首页实际生效的 `cd_setting.home_banner` 更新为 `/uploads/home_bg_20260610100529_118_147.jpg`，该文件与指定素材图片 SHA256 一致。
- 已校验 2026.6.6 新版 Excel 中“板块二、产业链服务”黄色标注的 6 个链接型单元格，当前数据库 `industry_chain` 对应 `link_url` 均与 Excel 一致。
- `ModuleDetailPage.jsx` 新增前台标题显示清理：左侧菜单、右侧阶段/分类/卡片标题、专题内容中展示节点标题时会移除开头数字序号；数据库原始节点标题不改，后台和接口数据保持不变。

相关验证：

- `frontend/src/tests/module-detail-service-card.test.jsx`
- 数据脚本校验：Excel 黄色标注 6 个链接型单元格无 mismatch。
