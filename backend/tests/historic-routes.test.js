...
describe('historic routes', () => {
  ...
  it('GET /api/admin/historic/nav-items should work', async () => {
    const res = await request(app).get('/api/admin/historic/nav-items')
    expect(res.status).toBe(200)
    expect(res.body.code).toBe(200)
  ...
  it('POST /api/admin/historic/nav-items should validate required fields', async () => {
    const res = await request(app).post('/api/admin/historic/nav-items').send({
      title: '',
      link_type: 'internal',
  ...
  it('admin nav CRUD should work', async () => {
    ...
    const listAfterCreate = await request(app).get('/api/admin/historic/nav-items')
    ...
    const listAfterUpdate = await request(app).get('/api/admin/historic/nav-items')
    const updated = listAfterUpdate.body.data.find((x) => x.id === created.id)
    expect(updated.title).toBe(updatedTitle)
    expect(updated.link_type).toBe('external')

    const deleteRes = await request(app).delete(`/api/admin/historic/nav-items/${created.id}`)
    expect(deleteRes.status).toBe(200)
    expect(deleteRes.body.code).toBe(200)

    const listAfterDelete = await request(app).get('/api/admin/historic/nav-items')
